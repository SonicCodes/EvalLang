...
function parse_complex(rule) {
  ...
  switch(operator){
    case "=" : {
      operation = {"operator":operator,"value":vars[1].replace(/^'|^"|"$|'$/gm,"")}
      break;
    }
    case "!=" : {
      operation = {"operator":"!=","value":vars[1].replace(/^'|^"|"$|'$/gm,"")}
      break;
    }
    case ">" : {
...
return parsed.map(rule_p=>{
  ...
  return is_a_rule ? parse_complex(rule) : {"field": rule, "is_set":true}
})
}
